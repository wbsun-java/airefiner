...
# Import common API error types for specific handling
try:
    from google.api_core.exceptions import ResourceExhausted
except ImportError:
    ResourceExhausted = None  # Graceful degradation
...
class ErrorHandler(LoggerMixin):
    ...
    def _get_user_friendly_message(self, error: Exception, context: str) -> str:
        ...
        # Handle our custom, wrapped error types first
        if isinstance(error, ProcessingError):
            # This error is a wrapper. The message was likely generated by a previous
            # call to this function on the original exception.
        ...
        # Handle raw, unwrapped exceptions below
        # This is where we can provide clean messages for common external errors

        # Specific check for Google's quota error
        if ResourceExhausted and isinstance(error, ResourceExhausted):
            return "Google API quota exceeded. Please wait and try again, or select a different model. The API suggests trying a newer model for higher limits."

        # Generic check for other rate limit errors (e.g., from OpenAI, Anthropic)
